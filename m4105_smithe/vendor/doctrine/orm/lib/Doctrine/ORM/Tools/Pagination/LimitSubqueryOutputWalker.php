...
class LimitSubqueryOutputWalker extends SqlWalker
{
    ...
    private function rebuildOrderByForRowNumber( SelectStatement $AST ) {
        ...
        foreach ( $orderByClause->orderByItems as $orderByItem ) {
            if ( is_string(
                    ...
                    && isset( $selectAliasToExpressionMap[ $orderByItem->expression ] )
            ) {
    ...
    private function addMissingItemsFromOrderByToSelect( SelectStatement $AST ) {
        ...
        foreach ( $selects as $idVar => $fields ) {
            $AST->selectClause->selectExpressions[] = new SelectExpression(
                    new PartialObjectExpression( $idVar, array_keys( $fields ) ), null, true
            );
        }
    }
            ...
            if ( isset( $fieldMapping[ 'declared' ] ) && $fieldMapping[ 'declared' ] !== $class->name ) {
                ...
                if ( !$otherClassMetadata->isMappedSuperclass ) {
                    $sqlTableAliasForFieldAlias = $this->getSQLTableAlias(
                            $otherClassMetadata->getTableName(), $dqlAliasForFieldAlias
                    );
    ...
    public function walkPathExpression( $pathExpr ) {
        if ( !$this->inSubSelect && !$this->platformSupportsRowNumber()
                && !in_array(
                        $pathExpr, $this->orderByPathExpressions
                )
        ) {
