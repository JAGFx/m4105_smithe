...
class LegacyDialogHelperTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSelectOnErrorOutput() {
        ...
        $this->assertEquals(
                '1', $dialog->select(
                $output = $this->getConsoleOutput( $this->getOutputStream() ), 'What is your favorite superhero?',
                $heroes, null, false, 'Input "%s" is not a superhero!', false
        )
        );

        rewind( $output->getErrorOutput()->getStream() );
        $this->assertContains(
                'Input "Stdout" is not a superhero!', stream_get_contents( $output->getErrorOutput()->getStream() )
        );
    }
    ...
    public function testAskOnErrorOutput() {
        ...
        $this->assertEquals(
                'not stdout', $dialog->ask(
                $output = $this->getConsoleOutput( $this->getOutputStream() ), 'Where should output go?', 'stderr'
        )
        );
    ...
    public function testAskHiddenResponseOnErrorOutput() {
        ...
        $this->assertEquals(
                '8AM', $dialog->askHiddenResponse(
                $output = $this->getConsoleOutput( $this->getOutputStream() ), 'What time is it?'
        )
        );
    ...
    public function testAskAndValidate()
    {
        ...
        try {
            $this->assertEquals(
                    'white', $dialog->askAndValidate(
                    $output = $this->getConsoleOutput( $this->getOutputStream() ), $question, $validator, 2, 'white'
            )
            );
            $this->fail();
        } catch (\InvalidArgumentException $e) {
            ...
            $this->assertContains(
                    ...
                    stream_get_contents( $output->getErrorOutput()->getStream() )
            );
        }
    }
